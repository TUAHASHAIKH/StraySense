...
const VaccinationSchedules = () => {
  ...
  useEffect(() => {
    const fetchAdoptedAnimals = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/user/adoptions', {
        ...
        const schedulesResponse = await axios.get('http://localhost:5000/api/vaccinations/animals', {
